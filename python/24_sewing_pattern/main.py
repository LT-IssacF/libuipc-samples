...
from uipc.core import Engine, World, Scene, Animation, SceneIO
from uipc.geometry import trimesh, tetmesh, label_surface, label_triangle_orient, flip_inward_triangles
from uipc.geometry import SimplicialComplexIO, AttributeIO
from uipc.constitution import (AffineBodyConstitution, NeoHookeanShell, DiscreteShellBending,
                               ElasticModuli, Empty, SoftPositionConstraint, SoftVertexStitch)
...
def simulate():
    ...
    t_shirt_front = io.read('./output_panel_top_front.obj')
    t_shirt_back = io.read('./output_panel_top_back.obj')
    ...
    dsb.apply_to(t_shirt_front, E=1e-3)
    dsb.apply_to(t_shirt_back, E=1e-3)
    t_shirt_front_elem.apply_to(t_shirt_front)
    t_shirt_back_elem.apply_to(t_shirt_back)

    PANEL_FILES = {
        'top_front': 'output_panel_top_front.obj',
        'top_back': 'output_panel_top_back.obj'
    }
    JSON_FILE_PATH = './output_stitch_data_local_indices.json'
    ...
    try:
        ...
        for stitch_info in all_stitch_data:
            ...
            if {p1_name, p2_name} == set(PANEL_FILES.keys()):
                ...
                print(f"  -> Connection found ({total_connections_found}): '{p1_name}' (edge {stitch_info['edge_1_index']}) <--> '{p2_name}' (edge {stitch_info['edge_2_index']})")
        ...
        if total_connections_found > 0:
            ...
            # if len(points1_np) > 0:
            #     print("\nExample:")
            #     print(f"First point pair:")
            #     print(f"  - Point on '{p1_print_name}' (index {indices1[0]}): {points1_np[0]}")
            #     print(f"  - Point on '{p2_print_name}' (index {indices2[0]}): {points2_np[0]}")
                
            #     print(f"Last point pair:")
            #     print(f"  - Point on '{p1_print_name}' (index {indices1[-1]}): {points1_np[-1]}")
            #     print(f"  - Point on '{p2_print_name}' (index {indices2[-1]}): {points2_np[-1]}")

    except (FileNotFoundError, Exception) as e:
        print(f"\nA fatal error occurred during file processing: {e}")

    # Check stitch vertices
    # cloth_front_trimesh = libtrimesh.load_mesh('./output_panel_top_front.obj')
    # cloth_back_trimesh = libtrimesh.load_mesh('./output_panel_top_back.obj')
    # cloth_front_verts = cloth_front_trimesh.vertices
    # cloth_back_verts = cloth_back_trimesh.vertices
    # cloth_front_color = np.zeros((cloth_front_verts.shape[0], 4), dtype=np.uint8)
    # cloth_back_color = np.zeros((cloth_back_verts.shape[0], 4), dtype=np.uint8)
    # cloth_front_color[indices1] = [255, 0, 0, 255]  # Red
    # cloth_back_color[indices2] = [0, 255, 0, 255]  # Green
    # cloth_front_trimesh.visual.vertex_colors = cloth_front_color
    # cloth_front_trimesh.export('./output_garment_panels_tops_colored.obj')
    # cloth_back_trimesh.visual.vertex_colors = cloth_back_color
    # cloth_back_trimesh.export('./output_garment_panels_tops_back_colored.obj')
    ...
    stitch_Vs = np.array([
        [i, j] for i, j in zip(indices1, indices2)
    ], dtype=np.int32)
    stitch_contact = scene.contact_tabular().create("stitch")
    front_contact_element_id = t_shirt_front.vertices().create('contact_element_id', t_shirt_front_elem.id())
    view(front_contact_element_id)[stitch_Vs[:,0]] = stitch_contact.id()
    back_contact_element_id = t_shirt_back.vertices().create('contact_element_id', t_shirt_back_elem.id())
    ...
    stitch_geo = svs.create_geometry((front_geo_slot, back_geo_slot), stitch_Vs, kappa=1e3)
    ...
    # scene.contact_tabular().insert(body_elem, t_shirt_front_elem, 0, 0, False)
    # scene.contact_tabular().insert(body_elem, t_shirt_back_elem, 0, 0, False)

    io = SimplicialComplexIO()
    body = io.read('./body.obj')
    label_surface(body)
    ...
    io = SimplicialComplexIO()
    io.write(f'output/cloth_front_surface{world.frame()}.obj', front_geo_slot.geometry())
    io.write(f'output/cloth_back_surface{world.frame()}.obj', back_geo_slot.geometry())
